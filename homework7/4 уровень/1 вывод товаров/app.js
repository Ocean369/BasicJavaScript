...
function showCategory(category) {
    divProduct.insertAdjacentHTML('beforeend', getProductMarkup(products[category]));
}
...
function getProductMarkup(product) {
    let url = 'https://example.com/producs/';
    let codeHTML = '';
    for (let i = 0; i < product.length; i++) {

        codeHTML += `<div class="product">
    <div>${product[i].name}</div>
    <img src="${product[i].imageUrl}" alt="">
    <div>${product[i].price}</div>
    <a href="${url}${product[i].id}">Подробнее</a>
</div>`}
    return codeHTML;
